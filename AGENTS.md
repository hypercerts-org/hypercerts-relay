...
Hypercerts Relay is a small, maintained fork of Bluesky Indigo plus a copied Jetstream v2 runtime module. The Indigo fork contains only Relay, Rainbow, and their Go dependency closure. The target service shape is:
...
The Relay publishes raw `com.atproto.sync.subscribeRepos` events. The `jetstream/` module owns retention of selected record collections and PDS/collection backfill. Rainbow is a raw event-stream proxy. Do not move those responsibilities between components without an explicit architecture decision.
...
- `jetstream` is a separately versioned Go module containing the copied Jetstream v2 runtime; read `jetstream/README.md` for provenance and its update procedure.
...
3. Identify whether the change belongs in the Relay, Rainbow, Jetstream v2, or the administration control plane. Implement Relay, Rainbow, and the copied Jetstream runtime here; keep OAuth/control-plane work separate.
...
- Only a human may merge a pull request. Do not directly push, fast-forward, reset, or force-update `main`, including while repairing a failed review flow.
