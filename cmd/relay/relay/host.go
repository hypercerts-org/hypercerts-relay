...
func (r *Relay) UpdateHostAccountLimit(ctx context.Context, hostID uint64, accountLimit int64) error {
	...
	// hypercerts: Reconcile remaining capacity on same-value retries after partial failure.
	if delta >= 0 {
		if err := r.reactivateQuotaAccounts(ctx, host.ID, accountLimit); err != nil {
			return err
		}
	}
	// Lower quotas only restrict future admission; existing accounts remain active.

	// hypercerts: Private management also operates when no slurper is attached.
	if r.Slurper != nil && r.Slurper.CheckIfSubscribed(host.Hostname) {
